...
We do not, however, claim these three points constitute a derivation. A rigorous first-principles calculation on the diamond lattice, producing $\Rzero^2$ as the unique shared-bond coherence factor from the LCE diagrammatic rules, remains an open problem. Until that derivation is completed, we present the 1.5~ppb result as a tightly-constrained plausibility argument; the 29~ppm bare-BKT result (no LCE at all) is the rigorous parameter-free headline claim.
